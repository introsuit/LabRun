...
namespace InputBlocker
{
    ...
    public class KeyboardListener : IDisposable
    {
        ...
        private bool ForbiddenInput(int vkCode, int flags)
        {
            Debug.WriteLine(vkCode + " " + flags);
            bool ctrlEsc = (vkCode == 27) && ctrl_pressed;
            bool winKey = (vkCode == 91);
            bool altEsc = (vkCode == 27) && (flags == 32);
            bool altF4 = (vkCode == 115) && (flags == 32);
            bool altTab = (vkCode == 9) && (flags == 32);
            bool ctrlW = (vkCode == 87) && ctrl_pressed;

            bool any = ctrlEsc || winKey || altEsc || altF4 || altTab || ctrlW;
            return any;
        }
    ...
    {
        public delegate int LowLevelKeyboardProc(int nCode, UIntPtr wParam, ref InterceptKeys.KBDLLHOOKSTRUCT lParam);
        ...
        [DllImport("user32.dll", EntryPoint = "CallNextHookEx", CharSet = CharSet.Ansi, SetLastError = true, ExactSpelling = true)]
        public static extern int CallNextHookEx(IntPtr hHook, int nCode, UIntPtr wParam, ref KBDLLHOOKSTRUCT lParam);
